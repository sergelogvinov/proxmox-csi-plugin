...
func buildMoveCmd() *cobra.Command {
	...
	cmd := cobra.Command{
		Use:           "storage pvc storage",
		Aliases:       []string{"s"},
		Short:         "Move data from one storage to another",
		Args:          cobra.ExactArgs(2),
		PreRunE:       c.moveValidate,
		RunE:          c.runMove,
		SilenceUsage:  true,
		SilenceErrors: true,
	}
...
func setMoveCmdFlags(cmd *cobra.Command) {
	flags := cmd.Flags()

	flags.StringP("namespace", "n", "", "namespace of the persistentvolumeclaims")

	flags.BoolP("force", "f", false, "force move even if the persistentvolumeclaims is in use")
	flags.Int("timeout", 7200, "task timeout in seconds")
}

// nolint: cyclop, gocyclo
func (c *moveCmd) runMove(cmd *cobra.Command, args []string) error {
	flags := cmd.Flags()
	...
	_, kubePV, err := tools.PVCResources(ctx, c.kclient, c.namespace, pvc)
	if err != nil {
	...
	logger.Infof("move disk %s to storage %s", vol.Disk(), storage)

	taskTimeout, _ := flags.GetInt("timeout") //nolint: errcheck
	if err = vm.MoveQemuDiskToStorage(cluster, vol, storage, taskTimeout); err != nil {
		return fmt.Errorf("failed to move disk: %v", err)
	}

	logger.Infof("replacing persistentvolume topology")

	// if err = replacePVTopology(ctx, c.kclient, c.namespace, kubePVC, kubePV, vol, vol.Node()); err != nil {
	...
	logger.Infof("persistentvolumeclaims %s has been migrated to storage %s", pvc, storage)

	return nil
}

// nolint: dupl
func (c *moveCmd) moveValidate(cmd *cobra.Command, _ []string) error {
	flags := cmd.Flags()
